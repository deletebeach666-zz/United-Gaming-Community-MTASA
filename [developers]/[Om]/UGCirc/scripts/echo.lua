...
addEventHandler("onPlayerLogin",root,
	function ()
		...
		messages[source] = 0
		outputIRC("*** "..getPlayerName(source).." joined the game. [Logging In]")
	end
)
...
addEventHandler("onPlayerQuit",root,
	function (quit,reason,element)
		...
		if reason then
			if element then
				outputIRC("*** "..getPlayerName(source).." was "..quit.." from the game by "..getPlayerName(element).." ("..reason..")")
			else
				outputIRC("*** "..getPlayerName(source).." was "..quit.." from the game by Console ("..reason..")")
			end
		else
			outputIRC("*** "..getPlayerName(source).." left the game ("..quit..")")
		end
	end
)
	...
	function (oldNick,newNick)
		...
		setTimer(function (player,oldNick)
			...
			if newNick ~= oldNick then
				outputIRC("* "..oldNick.." is now known as "..newNick)
			end
